...
// Vector layer: only Batas Kecamatan
...
(async function loadData(){
  try{
    ...
    // Ensure layer becomes visible in view by fitting once on load
    const extent = kecamatanLayer.getSource().getExtent();
    if(extent && isFinite(extent[0])){
      map.getView().fit(extent, { padding: [40, 40, 40, 40], duration: 300, maxZoom: 13 });
    }
  } catch(err){
    console.error('Gagal memuat batas_kecamatan.geojson', err);
  }
})();
